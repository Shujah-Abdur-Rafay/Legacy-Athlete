...
function buildAssessmentConfirmationEmail(parentName: string, athleteName: string, level: string, commitment: string, preferredDate: string): string {
  return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
function buildCoachAssessmentEmail(parentName: string, parentEmail: string, parentPhone: string, athleteName: string, grade: string, school: string, level: string, commitment: string, preferredDate: string, notes: string): string {
  return `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8">
<style>
...
function buildNewSignupEmail(email: string, displayName: string, uid: string, provider: string): string {
  return `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8">
<style>
...
export const submitAssessment = functions.https.onRequest((req, res) => {
  applyCors(req, res, async () => {
    if (req.method !== "POST" && req.method !== "OPTIONS") { res.status(405).json({ error: "Method not allowed" }); return; }

    const {
      athleteName, grade, school, level, commitment, preferredDate,
    ...
    if (!isValidEmail(String(parentEmail))) {
      res.status(400).json({ error: "Invalid parent email address" }); return;
    }
    if (!VALID_LEVELS.includes(String(level))) {
      res.status(400).json({ error: "Invalid level" }); return;
    }
    if (!VALID_COMMITMENTS.includes(String(commitment))) {
      res.status(400).json({ error: "Invalid commitment" }); return;
    }
    const preferredDateObj = new Date(String(preferredDate));
    if (isNaN(preferredDateObj.getTime())) {
      res.status(400).json({ error: "Invalid preferred date" }); return;
    }
    ...
    const preferredDateLabel = format(preferredDateObj, "EEEE, MMMM do, yyyy");
