...
function initStripe() {
    return new stripe_1.default(getStripeKey(), { apiVersion: "2024-04-10" });
}
function initResend() {
    return new resend_1.Resend(getResendKey());
}
/**
...
async function getOrCreateSharedCalendarId() {
    ...
    try {
        ...
        if (snap.exists && ((_a = snap.data()) === null || _a === void 0 ? void 0 : _a.calendarId)) {
            return snap.data().calendarId;
        }
    }
    catch (e) {
...
function generateICS(sessionDate, endDate, athleteName, focus, bookingId) {
...
function buildCoachEmail(athleteName, athleteEmail, sessionDate, focus, sessionId, bookingId, paymentStatus, calendarEventId) {
    const dateStr = (0, date_fns_1.format)((0, date_fns_tz_1.toZonedTime)(sessionDate, CST_TIMEZONE), "EEEE, MMMM do, yyyy");
    const timeStr = (0, date_fns_1.format)((0, date_fns_tz_1.toZonedTime)(sessionDate, CST_TIMEZONE), "h:mm a") + " CST";
    return `<!DOCTYPE html>
<html lang="en">
...
function buildWelcomeEmail(email, displayName) {
...
function buildPlanPurchaseEmail(email, planName, amount, transactionId, addons, is8Week) {
...
function buildCoachPlanEmail(userEmail, planName, amount, transactionId, addons, is8Week) {
...
function buildCancellationEmail(athleteName, sessionDate, focus) {
    const dateStr = (0, date_fns_1.format)((0, date_fns_tz_1.toZonedTime)(sessionDate, CST_TIMEZONE), "EEEE, MMMM do, yyyy");
    const timeStr = (0, date_fns_1.format)((0, date_fns_tz_1.toZonedTime)(sessionDate, CST_TIMEZONE), "h:mm a") + " CST";
    return `<!DOCTYPE html>
<html lang="en">
...
exports.exchangeCalendarToken = functions.https.onRequest((req, res) => {
...
exports.getCalendarStatus = functions.https.onRequest((req, res) => {
    applyCors(req, res, async () => {
        ...
        try {
            ...
            if (settingsDoc.exists && ((_a = settingsDoc.data()) === null || _a === void 0 ? void 0 : _a.refresh_token)) {
                const data = settingsDoc.data();
                res.json({
                    connected: true,
                    connectedAt: ((_c = (_b = data.connected_at) === null || _b === void 0 ? void 0 : _b.toDate()) === null || _c === void 0 ? void 0 : _c.toISOString()) || null,
            ...
            for (let i = 0; i < 14; i++) {
                ...
                if (daySchedule) {
                    for (const time of daySchedule.times) {
                        ...
                        const sessionDateUTC = (0, date_fns_tz_1.fromZonedTime)(sessionDateCST, CST_TIMEZONE);
 ...
 */
function hashCancellationToken(rawToken) {
...
async function checkRateLimit(key, maxRequests, windowSeconds) {
    ...
    try {
        return await db.runTransaction(async (tx) => {
            ...
            const requests = (((_a = docSnap.data()) === null || _a === void 0 ? void 0 : _a.requests) || [])
...
// ─── Audit logging ────────────────────────────────────────────────────────────
async function auditLog(action, actorId, details) {
    try {
        ...
        try {
            ...
            if (type === "booking") {
                ...
                if (!sessionId || !athleteName) {
                    res.status(400).json({ error: "sessionId and athleteName are required for bookings" });
                    return;
                }
                ...
                if (!planId) {
                    res.status(400).json({ error: "planId is required for plan purchases" });
                    return;
                }
                try {
                ...
                res.status(400).json({ error: "type must be 'booking' or 'plan'" });
                return;
            }
...
exports.createBooking = functions.https.onRequest((req, res) => {
    applyCors(req, res, async () => {
        ...
        if (String(athleteName).trim().length < 2) {
            res.status(400).json({ error: "Athlete name too short" });
            return;
        }
        ...
        try {
            ...
            if (paymentIntent.amount !== SESSION_PRICE_CENTS) {
                ...
                res.status(402).json({ error: "Payment amount does not match session fee" });
                return;
            }
            ...
            await db.runTransaction(async (transaction) => {
                ...
                if (bookedCount >= MAX_ATTENDANCE)
                    throw new Error("SESSION_FULL");
                sessionDate = new Date(decodeURIComponent(sessionId));
                if (isNaN(sessionDate.getTime()))
                    throw new Error("INVALID_SESSION_ID");
                const cstDate = (0, date_fns_tz_1.toZonedTime)(sessionDate, CST_TIMEZONE);
                const dayOfWeek = cstDate.getDay();
                const hours = cstDate.getHours();
                const minutes = cstDate.getMinutes();
                const daySchedule = SCHEDULE.find((s) => s.day === dayOfWeek);
                const timeSlot = daySchedule === null || daySchedule === void 0 ? void 0 : daySchedule.times.find((t) => t.h === hours && t.m === minutes);
                const duration = (timeSlot === null || timeSlot === void 0 ? void 0 : timeSlot.duration) || 60;
                sessionFocus = (timeSlot === null || timeSlot === void 0 ? void 0 : timeSlot.focus) || focusInput;
                ...
                cancellationToken = (0, uuid_1.v4)(); // raw — sent to athlete
                ...
                try {
                    ...
                    const event = await calendar.events.insert({
                        ...
                        requestBody: {
                            ...
                            description: `Athlete: ${safeAthleteName}\nEmail: ${String(email).toLowerCase().trim()}\nFocus: ${sessionFocus}\nDate: ${(0, date_fns_1.format)((0, date_fns_tz_1.toZonedTime)(sessionDate, CST_TIMEZONE), "EEEE, MMMM do, yyyy")}\nTime: ${(0, date_fns_1.format)((0, date_fns_tz_1.toZonedTime)(sessionDate, CST_TIMEZONE), "h:mm a")} – ${(0, date_fns_1.format)((0, date_fns_tz_1.toZonedTime)(endDate, CST_TIMEZONE), "h:mm a")} CST\nBooking ID: ${bookingId}`,
    ...
    applyCors(req, res, async () => {
        ...
        try {
            ...
            if (bookingQuery.empty) {
                res.status(404).json({ error: "Invalid or expired cancellation token" });
                return;
            }
            const bookingDoc = bookingQuery.docs[0];
            const booking = bookingDoc.data();
            // ── 24-hour cancellation window enforcement ───────────────────────────
            const sessionDate = ((_a = booking.session_date) === null || _a === void 0 ? void 0 : _a.toDate()) || new Date(decodeURIComponent(booking.session_id));
            ...
            await db.runTransaction(async (transaction) => {
                var _a;
                const sessionDoc = await transaction.get(sessionDocRef);
                const newCount = Math.max(0, ((sessionDoc.exists ? (_a = sessionDoc.data()) === null || _a === void 0 ? void 0 : _a.bookedCount : 1) || 1) - 1);
...
exports.confirmPlanPurchase = functions.https.onRequest((req, res) => {
    applyCors(req, res, async () => {
        if (req.method !== "POST" && req.method !== "OPTIONS") {
            res.status(405).json({ error: "Method not allowed" });
            return;
        ...
        if (!email || !planName || !amount || !transactionId) {
            res.status(400).json({ error: "Missing required fields: email, planName, amount, transactionId" });
            return;
        }
        ...
        try {
            ...
            if (paymentIntent.status !== "succeeded") {
                res.status(402).json({ error: "Payment not confirmed by Stripe. Status: " + paymentIntent.status });
                return;
            }
...
exports.checkCancellation = functions.https.onRequest((req, res) => {
    applyCors(req, res, async () => {
        var _a;
        ...
        try {
            ...
            res.json({
                ...
                focus: booking.focus,
                date: (0, date_fns_1.format)((0, date_fns_tz_1.toZonedTime)(sessionDate, CST_TIMEZONE), "EEEE, MMMM do, yyyy"),
                time: (0, date_fns_1.format)((0, date_fns_tz_1.toZonedTime)(sessionDate, CST_TIMEZONE), "h:mm a") + " CST",
                bookingId: booking.id,
...
exports.getSharedCalendarEvents = functions.https.onRequest((req, res) => {
    applyCors(req, res, async () => {
...
exports.getSharedCalendarIdEndpoint = functions.https.onRequest((req, res) => {
    applyCors(req, res, async () => {
        try {
            const calendarId = await getOrCreateSharedCalendarId();
            res.json({ calendarId });
        ...
        switch (event.type) {
                    ...
                    if (bookingQuery.empty && sessionId && athleteName && email) {
                        ...
                        try {
                            await db.runTransaction(async (tx) => {
                                ...
                                const timeSlot = daySchedule === null || daySchedule === void 0 ? void 0 : daySchedule.times.find((t) => t.h === cstDate.getHours() && t.m === cstDate.getMinutes());
                                const duration = (timeSlot === null || timeSlot === void 0 ? void 0 : timeSlot.duration) || 60;
                                sessionFocus = (timeSlot === null || timeSlot === void 0 ? void 0 : timeSlot.focus) || "Training Session";
                                endDate = new Date(sessionDate.getTime() + duration * 60 * 1000);
                                bookingId = (0, uuid_1.v4)();
                                cancellationToken = (0, uuid_1.v4)(); // raw — goes in email URL
                            ...
                            if (calendar) {
                                try {
                                    ...
                                    console.error("[webhook] Calendar error:", calErr);
                                }
                            }
                            const resend = initResend();
                            ...
                            if (txErr.message === "SESSION_FULL") {
                                // Session full — refund
                                try {
                                    await stripe2.refunds.create({ payment_intent: pi.id, reason: "duplicate" });
                                }
                                catch (_f) { }
                            }
            ...
            case "payment_intent.payment_failed": {
                ...
                await db.collection("payment_failures").add({
                    paymentIntentId: pi.id,
                    errorCode: ((_a = pi.last_payment_error) === null || _a === void 0 ? void 0 : _a.code) || null,
                    errorMessage: ((_b = pi.last_payment_error) === null || _b === void 0 ? void 0 : _b.message) || null,
            ...
            case "charge.dispute.created": {
                ...
                const piId = typeof dispute.payment_intent === "string"
                    ...
                    : ((_c = dispute.payment_intent) === null || _c === void 0 ? void 0 : _c.id) || "";
                ...
                await db.collection("disputes").add({
                    ...
                    evidenceDueBy: ((_d = dispute.evidence_details) === null || _d === void 0 ? void 0 : _d.due_by)
                ...
                try {
                   ...
                   Evidence due by: <b>${((_e = dispute.evidence_details) === null || _e === void 0 ? void 0 : _e.due_by)
                   ...
                   Respond in the <a href="https://dashboard.stripe.com/disputes/${dispute.id}">Stripe Dashboard</a>.</p>`,
                    ...
                    console.error("[webhook] Dispute email failed:", emailErr);
                }
                break;
            }
            default:
...
exports.setAdminClaim = functions.https.onRequest((req, res) => {
    applyCors(req, res, async () => {
        ...
        if (!bootstrapSecret) {
            res.status(500).json({ error: "Admin bootstrap secret not configured on server" });
            return;
        }
        ...
        if (!targetEmail || !isValidEmail(String(targetEmail))) {
            res.status(400).json({ error: "Valid targetEmail required" });
            return;
        }
        try {
